...
std::atomic<int> counter(0);  // Bộ đếm chung

// Hàm thực hiện Fetch-and-Add với Exponential Backoff
void increment_fetch_add_with_backoff(int times) {
    int backoff = 1;                 // Thời gian backoff ban đầu (ns)
    const int MAX_BACKOFF = 1 << 15; // Giới hạn tối đa thời gian backoff (ns)

    for (int i = 0; i < times; ++i) {
        int current_value = counter.load();  // Đọc giá trị hiện tại của bộ đếm
        while (counter.fetch_add(1, std::memory_order_relaxed) != current_value) {
            // Nếu fetch_add thất bại (do tranh chấp), thử lại sau khi backoff
...
bool test_correctness(int num_threads, int times) {
    ...
    for (int i = 0; i < num_threads; ++i) {
        threads.push_back(std::thread(increment_fetch_add_with_backoff, times));
    }
